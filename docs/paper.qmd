...
Some are mere extensions of their categories (e.g., specifying if an innovation prolongs the life cycle of the product itself or downstream products), others are new types of existing categories (e.e.g., reduced other pollution such as noise), yet others are entirely new categories (human health and safety).
...
For example, XX innovations aim to protect harvested timber, either at harvest, during transport or storage.
...
The only down side to the articles approach is a quintupling of labeling time (from 1 min on average to 5 min on average per innovation).
...
While still early, a first analysis of the labeling results in regards to trends over time and across sectors follows.
...
Partially this might stem from a small number of innovations labeled so far, however, no clearer pattern emerges in the larger dataset using only SWINNO either (@fig-bv-years).
...
Looking at the eco-innovation types by sector reveals that the biggest category and most heavily used in the forest and timber sector are innovations aimed to prolong the lifecylce of products used therin (@fig-ei-sectors)
```{python}
...
For the count of eco-innovation types by year, the legend had to be surpressed, as the amount of categories made the plots unreadable.
...
The subset of innovations in SWINNO analyzed is too small to descern any real trends.
...
Perhpas surprising is that, despite the small number of innovation labelled so far and the methodological disadvantage of LBIO towards bioecology innovations, three were possibly already identified.
There occurance in the 1980s might suggest taht they were the outcome of a temporary increased awareness of environmental issues in the forestry sector at that time.
