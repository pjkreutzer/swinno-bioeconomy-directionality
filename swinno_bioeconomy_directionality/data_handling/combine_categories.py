...
from swinno_bioeconomy_directionality.config import RAW_DATA_DIR, INTERIM_DATA_DIR
...
if __name__ == "__main__":
    ...
    for csv in Path(RAW_DATA_DIR, "tags").glob("*.csv"):
    ...
    for combined_df in [combined_innovation_types, combined_notes, combined_visions]:
        csv_path = Path(INTERIM_DATA_DIR, f"{combined_df.name}-duplicates.csv")
        check_duplicates(
            combined_df, output_name=combined_df.name, output_path=csv_path
        )
        print("-" * 5)

    split_visions = split_cols(
        combined_visions, col_to_split="bioeconomy_vision", sep=","
    )
    melted_visions = melt_table(
        split_visions,
        id_vars="sinno_id",
        col_start="bio",
        value_name="bioeconomy_vision",
    )
