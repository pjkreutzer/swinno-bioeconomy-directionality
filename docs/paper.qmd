...
In addition to the 30 firm performance indicators identified by García-Granero's [-@garcia-granero2018EcoinnovationMeasurementReview] review of management literature, 15 categories were added.
Some are mere extensions of their categories (e.g., specifying if an innovation prolongs the life cycle of the product itself or downstream products), others are new types of existing categories (e.e.g., reduced other pollution such as noise), yet others are entirely new categories (human health and safety), the categories of automatization and precision by themselves are not eco-innovations, but can be used to gain a deeper understanding of what makes an innovation an eco-innovation.
...
An innovation is tagged as belonging to a bioeconomy vision if it clearly fulfills at least one of the criteria unique to the vision discussed in Bugge et al.[-@bugge2016WhatBioeconomyReview, @tbl-bv-bugge] and Vivien et al. [-@vivien2019HijackingBioeconomy, @tbl-bv-vivien].
...
If an innovation only aims to upgrade a bioresource, it is tagged as belonging to the bioresource vision, whereas if these products also feature a territorial identity, it would be tagged as both bioresource and bioecology vision.
Of course there are innovations belonging to the forestry sector which fit into no vision.
<!-- TODO: Include Examples -->


For both classifications, uncertain cases were tagged to allow for later review, with intentions to use consensus between experts for final decision-making.
...
| Value   Creation | Application of biotechnology,   commercialization of research & technology | Conversion and upgrading of   bio-resources (process oriented) | Development of integrated   production systems and high-quality products with territorial identity |
| Drivers   & mediators of innovation | R & D, patents, TTOs,   Research councils and funders (Science push, linear model) | Interdisciplinary, optimization   of land use, include degraded land in the production of biofuels, use and   availability of bio-resources, waste management, engineering, science &   market (Interactive & networked production mode) | Identification of favorable   organic agro-ecological practices, ethics, risk, transdisciplinary   sustainability, ecological interactions, re-use & recycling of waste,   land use, (Circular and self-sustained production mode) | 
...
While this does inflate the number of non-applicable labels, it is perhaps a good sign that the labels are indeed useful and accurate when differentiating between eco-innovations / bioeconomy related innovations and those that do not belong to these categories.

No additional steps were taken at this point to remedy the error, instead this sector was simply excluded before moving on the classification using source articles.
...
## Classifications Using Source Articles
...
Beginning with the innovations tagged as Unsure in the SWINNO only classification, the source articles were read and tags for bioeconomy visions and eco-innovations given on this basis.
@tbl-bv-article-count and @tbl-ei-article-count show the results thus far.

```{python}

def label_confidence(df):
  count_unsure = df.loc[df.iloc[:, 0] == "Unsure", "Count"]
  total_labels = df.iloc[:, 1].sum()
  return float(1 - (count_unsure / total_labels).iloc[0]) #to return the value instead of series

count_dfs_confidence = {"ei_count": ei_count, "ei_count_articles":ei_count_articles, "bv_count":bv_count, "bv_count_articles":bv_count_articles}

label_confidences = {k: label_confidence(v) for  k, v in count_dfs_confidence.items()}


display(Markdown(f"The confidence of labels for each approch, measured as the share of Unsure to total classifications is present in each data set shows that relying on source articles drastically reduces the uncertainty of labelling decisions(Eco-Innovation: {label_confidences['ei_count']:.2%} to {label_confidences['ei_count_articles']:.2%} and Bioeconomy: {label_confidences['bv_count']:.2%} to {label_confidences['bv_count_articles']:.2%}, for SWINNO and articles, respectively)."))
```

The only downside to the articles approach is a quintupling of labeling time (from 1 min on average to 5 min on average per innovation).

```{python}
#| label: tbl-bv-article-count
#| tbl-cap: Count of Bioeconomy Visions Using Articles
...
While still early, a first analysis of the labeling results regarding trends over time and across sectors follows.
In this section only data based on source articles is presented.
The figures and tables for SWINNO articles are included in the appendix.

## Bioeconomy Vision

Examining the split of bioeconomy visions across sectors in @fig-bva-sectors, reveals a strong concentration of bioresource related innovations in the forestry sector.
...
bva_sectors = pd.read_sql(
    """
SELECT
  c.Category as vision,
  sni_codes.label as sector,
  count(bva.bioeconomy_vision) as count
FROM
  bioeconomy_visions_articles as bva
  join use_sectors ON bva.sinno_id = use_sectors.[SINNO ID]
  join sni_codes ON SUBSTR(use_sectors.use_sectors, 1, 2) = sni_codes.code
  join [classification-codes] as c ON bva.bioeconomy_vision = c.Code
WHERE 
  NOT EXISTS(
   SELECT
      *
    FROM
      bioeconomy_visions_articles AS bva2
    WHERE
      bva2.sinno_id = bva.sinno_id
      AND bva2.bioeconomy_vision = 0
)
GROUP BY
  sector,
  bioeconomy_vision;
...
bva_matrix = prepare_heatmap_data(
    bva_sectors, index_col="sector", cols_col="vision", values_col="count"
)

plot_heatmap(bva_matrix, "Count Bioeconomy Vision by Sector Excluding Uncertains")
```

@fig-bva-years suggests no clear pattern in the absolute count of visions assigned to innovations.
Partially this might stem from the small number of innovations labeled so far, however, no clearer pattern emerges in the larger dataset using only SWINNO either (@fig-bv-years).
The small number of biotechnology related visions is due to the chemistry sector not yet being included in the labelling.
...
Looking at the eco-innovation types by sector reveals that the biggest category and most heavily used in the forest and timber sector are innovations aimed to prolong the lifecycle of products used therein (@fig-ei-sectors)
```{python}
#| label: fig-eia-sectors
#| fig-cap: Heatmap of Eco-Innovations by Sector


eia_sectors = pd.read_sql(
    """
SELECT
  c.Category as innovation_type,
  sni_codes.label as sector,
  count(eia.innovation_type) as count
FROM
  eco_innovations_articles as eia
  join use_sectors ON eia.sinno_id = use_sectors.[SINNO ID]
  join sni_codes ON SUBSTR(use_sectors.use_sectors, 1, 2) = sni_codes.code
  join [classification-codes] as c ON eia.innovation_type = c.Code
WHERE 
  NOT EXISTS(
   SELECT
      *
    FROM
      eco_innovations_articles AS eia2
    WHERE
      eia2.sinno_id = eia.sinno_id
      AND eia2.innovation_type = "000"
)
GROUP BY
  sector,
  innovation_type;
...
eia_sectors_matrix = prepare_heatmap_data(
    eia_sectors, index_col="sector", cols_col="innovation_type", values_col="count"
)

plot_heatmap(eia_sectors_matrix, title="Eco-Innovation Type by Sector")

```

For the count of eco-innovation types by year, the legend had to be suppressed, as the amount of categories made the plots unreadable.
Just as for bioeconomy visions @fig-eia-year shows no clear pattern yet.
@fig-ei-year-swinno does little to improve this.
The biggest categories in that dataset are "Not Eco-Innovation" (dark blue), and "Automatization" (dark pink).
Visualizing these data is challenging and suggestions warmly welcome.

```{python}
#| label: fig-eia-year
#| fig-cap: Count of Eco-Innovations by Year

eia_years = pd.read_sql(
    """
SELECT
  codes.Category AS Type,
  count(eia.innovation_type) AS innovation_count,
  innovation.[Year of commercialization] AS Year,
  SUBSTR(eia.innovation_type, 1,1) as top_cat
FROM
  eco_innovations_articles AS eia
  JOIN innovation ON eia.sinno_id = innovation.[SINNO ID]
  JOIN [classification-codes] AS codes ON eia.innovation_type = codes.Code
WHERE
  TRIM(innovation.[Year of commercialization]) IS NOT ""
GROUP BY
  Year,
...
SELECT
  ...
  count(eia.innovation_type) AS innovation_count,
  innovation.[Year of commercialization] AS year,
  SUBSTR(eia.innovation_type, 1,1)
FROM
  eco_innovations_articles AS eia
  JOIN innovation ON eia.sinno_id = innovation.[SINNO ID]
  JOIN [classification-codes] AS codes ON eia.innovation_type = codes.Code
...
So far, there is not much to make of these figures.
The subset of innovations in SWINNO analyzed is too small to discern any real trends.
However, there is probable cause to believe that the tendency of dominating bioresource vision related innovations will persist.
It is unsurprising that in the core forestry sector (logging, timber) these type of innovations are more frequent than e.g., biotechnology related visions.
Perhaps surprising is that, despite the small number of innovation labelled so far and the methodological disadvantage of LBIO towards bioecology innovations, three were possibly already identified.
There occurrence in the 1980s might suggest that they were the outcome of a temporary increased awareness of environmental issues in the forestry sector at that time.

## Relation between Eco-Innovations and Bioeconomy Visions
...
<!-- ```{python}
#| label: tbl-bioeco-examples
#| tbl-cap: Examples of Bioecology Vision

bv3_examples = pd.read_sql(
  """
  SELECT
  bv.sinno_id AS [SINNO ID],
  innovation.[Description in Swedish]
FROM
  bioeconomy_visions_articles AS bv
JOIN innovation ON bv.sinno_id = innovation.[SINNO ID]
WHERE
bv.bioeconomy_vision = 3 
AND
  NOT EXISTS (
    SELECT
      *
...
styled_df = bv3_examples.style.set_properties(subset=[bv3_examples.columns[-1]], **{'width': '70%', 'max-width': '80%', 'text-align': 'justify', 'word-wrap': 'break-word', 'white-space': 'pre-wrap'})

# display the styled dataframe
display(Markdown(styled_df.hide_index().render())) 

```-->

| SINNO ID | Description in Swedish |
|:---|:---|
| 7440001  | Trämelass. Trämelassen består av de mest lättlösliga substanserna i vedråvaran för skivframställning  vilka går i lösning i tillverkningsprocessen och inblandas i processvattnet. Sedan mitten av 1960-talet återvinnes substanserna koncentreras och torkas samt ges en viss efterbehandling som gör dem lämpliga som bl a kreatursfoder. Produkten är alltså dels rent kreatursfoder och dels en ingående ingredients i kreatursfoder. |
| 7441001  | Maskinsystem för småskalig avverkning av energiskog (ESO).                                                          
| 7446001  | Maskinsystem för storskalig avverkning av energiskog (ESO) i företrädesvis våtmark. I systemet ingår både skördare och buntare. |                                                      
| 7508001  | Elsåg för motorsågsdrivna sågverk. Elsågen håller en jämnare hastighet jämfört med en dieseldriven såg det medför lägre bränslekostnader.
| 7555001  | Patenterad lösning för stegdämpning i trähus. Innovationen består av en extremt kraftig gummilist som tar upp vibrationer. |
: Examples of Bioecology Vision {#tbl-bioeco-examples tbl-colwidths="[15,85]"}                                     

@tbl-bioeco-examples shows the five innovations tagged without uncertainty as belonging to the bioecology vision.
While the first is perhaps the clearest example, the other three are not as obvious.
After some deliberation, I decided that they ought to belong to the bioecology vision, as they all include strong territorial identity.
But I have doubts if that is true enough.

Further, doubts regarding the overall approach come from the fact that it feels very subjective at times, and I struggle with designing a more rigorous approach to classification of bioeconomy visions.

## Thoughts

At times, it can be difficult to decide whether an innovation should be counted as optimization of raw material, reduction of waste, or both.
...
```{python}
#| label: fig-bv-sectors
#| fig-cap: Heatmap of Bioeconomy Visions by Sector (SWINNO)
...
#### Bioeconomy Visions Over Time

```{python}
#| label: fig-bv-years
#| fig-cap: Count of Bioeconomy Visions by Year (SWINNO)
...
#### Eco-Innovations by Sector

```{python}
#| label: fig-ei-sectors
#| fig-cap: Heatmap of Eco-Innovations by Sector (SWINNO)
...
```{python}
#| label: fig-ei-year-swinno
#| fig-cap: Count of Eco-Innovations by Year (SWINNO).

ei_years = pd.read_sql(
    """
SELECT
  codes.Category AS Type,
...
ei_years = ei_years.set_index("Year")
eic_years = eic_years.set_index("year")
